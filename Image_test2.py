...
img2 = cv2.imread("bc1a411e-4931-4390-a4ce-5c504541c4ea (1).jpg")
...
# Định nghĩa các tập ký tự hợp lệ (ASCII)
# Hàng trên: Số (0-9) và Chữ (A-Z)
intValidChars_Top = [ord('0'), ord('1'), ord('2'), ord('3'), ord('4'), ord('5'), ord('6'), ord('7'), ord('8'), ord('9'),
                     ord('A'), ord('B'), ord('C'), ord('D'), ord('E'), ord('F'), ord('G'), ord('H'), ord('I'), ord('J'),
                     ord('K'), ord('L'), ord('M'), ord('N'), ord('O'), ord('P'), ord('Q'), ord('R'), ord('S'), ord('T'),
                     ord('U'), ord('V'), ord('W'), ord('X'), ord('Y'), ord('Z')]
# Hàng dưới: Chỉ Số (0-9)
intValidChars_Bottom = [ord('0'), ord('1'), ord('2'), ord('3'), ord('4'), ord('5'), ord('6'), ord('7'), ord('8'), ord('9')]
...
for c in contours:
    ...
    # Lấy biển số y như code1 – không loại thêm lọc nào
    if len(approx) == 4:
...
if detected == 1:
    ...
    for screenCnt in screenCnt:
        ...
        # Lấy tọa độ cho hiển thị kết quả sau (nếu cần)
        (x_min, y_min) = np.min(screenCnt[:, 0, :], axis=0)
        (x_max, y_max) = np.max(screenCnt[:, 0, :], axis=0)
        topy = y_min
        topx = x_min
        ...
        ke = abs(x1 - x2)
        if ke == 0: angle = 90.0
        else: angle = math.atan(doi / ke) * (180.0 / math.pi)
        ...
        if len(x) == 0 or len(y) == 0: continue # Bỏ qua nếu contour lỗi
        ...
        if h_roi == 0 or w_roi == 0: continue # Bỏ qua nếu roi lỗi
        ...
        roi = cv2.warpAffine(roi, rotationMatrix, (w_roi, h_roi))
        imgThresh = cv2.warpAffine(imgThresh, rotationMatrix, (w_roi, h_roi))
        ...
        for i in char_x:
            ...
            intCurrentChar = int(npaResults[0][0])
            strCurrentChar = str(chr(intCurrentChar))   # Lấy ký tự nhận dạng ban đầu
            
            # --- LOGIC LỌC KÝ TỰ ĐÃ CHỈNH SỬA ---
            bCharIsValid = False
            
            if (y < height / 3):  # Ký tự thuộc Hàng trên
                if intCurrentChar in intValidChars_Top:
                    first_line = first_line + strCurrentChar
                    bCharIsValid = True
            else:                 # Ký tự thuộc Hàng dưới (chỉ chấp nhận số)
                if intCurrentChar in intValidChars_Bottom:
                    second_line = second_line + strCurrentChar
                    bCharIsValid = True

            # Chỉ hiển thị và in ra ký tự hợp lệ
            if bCharIsValid:
                cv2.putText(roi, strCurrentChar, (x, y + 50), cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 0), 3)
            # --- KẾT THÚC LOGIC LỌC KÝ TỰ ---


        print("\n License Plate " + str(n) + " is: " + first_line + " - " + second_line + "\n")
        
        # Thêm hiển thị kết quả lên ảnh gốc
        strResult = first_line + " - " + second_line
        cv2.putText(img, strResult ,(topy ,topx),cv2.FONT_HERSHEY_DUPLEX, 2, (0, 255, 255), 3)
