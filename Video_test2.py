...
# *** KIỂM TRA MỞ VIDEO BAN ĐẦU ***
if not cap.isOpened():
    print("Lỗi: Không thể mở video. Vui lòng kiểm tra lại đường dẫn: 'data/video/video3.mp4'")
    exit()
# **********************************
...
while (cap.isOpened()):
    ...
    # *** KHẮC PHỤC LỖI 'NoneType' ***
    if not ret or img is None:
        print("Đã kết thúc video hoặc lỗi đọc frame, thoát vòng lặp.")
        break
    # ********************************
    
    tongframe = tongframe + 1
    # img = cv2.resize(img, None, fx=0.5, fy=0.5) # Nếu bạn uncomment dòng này, hãy nhớ kiểm tra lại các ngưỡng Min/Max area
    ...
    # Sửa: Đặt biến topx và topy mặc định để tránh NameError nếu không tìm thấy biển số
    # Các giá trị này sẽ được cập nhật trong vòng lặp for bên dưới nếu tìm thấy biển số
    topy = 0
    topx = 0
    ...
    if detected == 1:
        ...
        for screenCnt in screenCnt:
            ...
            if ke == 0:
                angle = 0.0 # Gán góc bằng 0 nếu không có sự thay đổi tọa độ x (biển số thẳng đứng)
            else:
            ...
            # Bỏ qua nếu contour lỗi không có pixel 
            ...
            # Bỏ qua nếu kích thước ROI không hợp lệ
            ...
            # Lấy kích thước hiện tại của ROI sau khi cắt
            h_roi, w_roi = bottomx - topx + 1, bottomy - topy + 1
    ...
    # Kiểm tra để tránh lỗi nếu imgcopy là None (mặc dù đã xử lý ở đầu vòng lặp)
    if img is not None:
        ...
        # Sửa lỗi chia cho 0 nếu tổng frame bằng 0, mặc dù tongframe luôn > 0 trong vòng lặp
        if tongframe > 0:
            print("plate found rate:", 100 * biensotimthay / (368), "%")
        else:
            print("plate found rate: 0 %")
